...
function save_hotel(){
	...
	$check = $this->db->query("SELECT * FROM hotel where email ='$email' ".(!empty($id) ? " and id != {$id} " : ''))->num_rows;
	if($check > 0){
		return 2;
		exit;
	}
	...
	if(empty($id)){
		$save = $this->db->query("INSERT INTO hotel set $data");
	}else{
		$save = $this->db->query("UPDATE hotel set $data where id = $id");
	}
	if($save){
		return 1;
	}
...
function transport(){
	...
	$check = $this->db->query("SELECT * FROM transport where email ='$email' ".(!empty($id) ? " and id != {$id} " : ''))->num_rows;
	if($check > 0){
		return 2;
		exit;
	}
	...
	if(empty($id)){
		$save = $this->db->query("INSERT INTO transport set $data");
	}else{
		$save = $this->db->query("UPDATE transport set $data where id = $id");
	}
	if($save){
		return 1;
	}
...
// ende transport file
 ...
 function delete_hotel(){
	extract($_POST);
	$delete = $this->db->query("DELETE FROM hotel where id = ".$id);
	if($delete)
		return 1;
}
	...
	function save_image(){
		...
		if(!empty($tmp_name)){
			...
			$move = move_uploaded_file($tmp_name,'assets/uploads/'. $image);
