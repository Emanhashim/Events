		...
		<div class="card-body">
			<table class="table tabe-hover table-bordered" id="list">
				...
				<tbody>
					...
					$qry = $conn->query("SELECT *,concat(transport_type,' ',transport_type) as name FROM transport order by concat(transport_type,' ',transport_type) asc");
					// $qry = $conn->query("SELECT * FROM filing ");
					while($row= $qry->fetch_assoc()):
					?>
					<tr>
						<th class="text-center"><?php echo $i++ ?></th>
						<td><b><?php echo ucwords($row['transport_type']) ?></b></td>
